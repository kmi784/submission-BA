...
{
    // stores current state of PRNG-engine
    os << s.gen << " ";
...
{
    // loads current state of PRNG-engine
    is >> s.gen;
    ...
    for (unsigned int i = 0; i < sweeps; ++i) {
        for (int j = 0; j < L * L; ++j) {
            ...
            if (dEnergy <= 0) {
                singleflip(x, y);
                Energy += dEnergy;
            } else if (Uc(gen) < * (boltzmannweights + 5 * (dFaktor.first / 4 + 2) + dFaktor.second /
                                    4
                                    + 2)) {
                singleflip(x, y);
                Energy += dEnergy;
...
// counting energy histogram and magnetization-lists
void System::count(Histogram& h)
{
    if (h.size() == 0) {
        h.push_back(
            Energy, 1,
...
// loading energy histogram and magnetization lists
...
// storing energy histogram and magnetization lists
